...
This command leverages the Docker daemon to push an entire mono-repo's Docker images to a repository. In order to use this command, a Docker images must have already been generated by Orphedomos. 

Example Syntax: 

*CLI:* 
```
mvn orphedomos:push-all-docker-images -Dorphedomos.repository.url=<url/to/docker/repository> -Dorphedomos.repository.id=<repo-id> -Dorphedomos.credential.usePlainTextPassword=true
```
